...
func (img *Image) Fullfill(domain string) {
...
func (img *Image) Scan(value interface{}) error {
	bytes, ok := value.([]byte)
	if !ok {
		return errors.WithStack(errors.New(fmt.Sprintf("Failed to unmarshall data from DB: %s", value)))
	}
...
func (i *Images) Scan(value interface{}) error {
	bytes, ok := value.([]byte)
	if !ok {
		return errors.WithStack(errors.New(fmt.Sprintf("Failed to unmarshall JSONB value: %s", value)))
	}

	var data []Image
	if err := json.Unmarshal(bytes, &data); !ok {
		return errors.WithStack(err)
	}
